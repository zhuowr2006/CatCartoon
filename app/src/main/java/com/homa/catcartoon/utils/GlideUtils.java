...
public class GlideUtils {
    ...
    public void LoadContextBitmap(Context context, int id, ImageView imageView, int placeid, int errorid, String bitmapOrgif){
        if(bitmapOrgif==null||bitmapOrgif.equals(LOAD_BITMAP)){
            Glide.with(context).load(id).placeholder(placeid).error(errorid).crossFade().into(imageView);
        }else if(bitmapOrgif.equals(LOAD_GIF)){
            Glide.with(context).load(id).asGif().crossFade().into(imageView);
        }
    }
    ...
    public void LoadContextCircleBitmap(Context context, int id, ImageView imageView){
        Glide.with(context).load(id).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
    }
    /**
